...
when-to-use:
- diagnose this, debug this, something is broken/throwing/failing
- Performance regressions; hard or flaky single bugs (vs bug-hunt's discovery sweeps)
permissions:
- read
- write
- bash
...
A discipline for hard bugs: reproduce → minimise → hypothesise → instrument → fix → regression-test. Skip phases only when explicitly justified.

If the repo keeps a domain glossary (`CONTEXT.md`) or ADRs (`docs/adr/`), read the ones touching the affected area before exploring.
...
- [ ] `ubs` run on changed files

**Then ask: what would have prevented this bug?** If the answer involves architectural change (no good test seam, tangled callers, hidden coupling), hand off to the `improve-codebase-architecture` skill with the specifics. Make the recommendation **after** the fix is in, not before — you have more information now than when you started.

## Scope note

This skill is for **one hard bug**. For breadth-first discovery sweeps across a codebase, use `bug-hunt` or `multi-pass-bug-hunting` instead.
